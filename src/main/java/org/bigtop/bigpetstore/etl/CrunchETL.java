...
public class CrunchETL extends PetStoreStatistics {
    ...
    //STATIC function declarations, so that we dont run into serialization reference issues.
    ...
    public CrunchETL(Path input, Path output) throws Exception {
        ...
        PCollection<String> lines = 
                pipeline.read(
                        From.textFile(new Path(input,"part-r-00000")));
        System.out.println("crunch : "+ lines.getName() +  "  " + lines.getSize());
        //System.out.println(FileSystem.get(new Configuration()).getLength(input));
    ...
    public Map<String, ? extends Number> numberOfTransactionsByState()
            throws Exception {
        PTable<String, Long> counts = lineItems.parallelDo(
                COUNT_BY_STATE, Avros.strings()).count();
    ...
    public Map<String, ? extends Number> numberOfProductsByProduct()
            throws Exception {
        PTable<String, Long> counts = lineItems.parallelDo(
                COUNT_BY_PRODUCT, Avros.strings()).count();
         Map m = counts.materializeToMap();
         System.out.println("Crunch:::  " + m);
         return m;
    }
...
/**
        PCollection<String> lines = MemPipeline
                .collectionOf(
...
**/
        ...
        PCollection<String> lines = pipeline.read(
                From.textFile(
                        new Path("/tmp/BigPetStore1388719888255/generated/part-r-00000")));
        ...
        /**
        //System.out.println();
        final PTable<String, Long> counts = lineItems.parallelDo(
                new MapFn<LineItem, String>() {
                    public String map(LineItem lineItem) {
                        try{
                            System.out.println("proc2");
                            return lineItem.getAppName();
                        }
                        catch(Throwable t){
                            throw new RuntimeException(t);
                        }
       ...
       for(LineItem i : lineItems.materialize())
        ...
        //System.out.println(lineItems.getSize() + " " + lineItems.count() + " " + lineItems.asCollection().getValue().size());
    }
